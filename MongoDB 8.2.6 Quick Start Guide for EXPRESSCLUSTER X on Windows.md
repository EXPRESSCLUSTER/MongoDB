MongoDB 8.2.6 Quick Start Guide for EXPRESSCLUSTER X on Windows
===

About this guide
---
This guide describes how to setup MongoDB 8.2.6 with EXPRESSCLUSTER X 5.3.
...
- failover Group: `MongoDBfailover`
	- Group resource
		- Fip                  : Floating IP address resource
		- md                   : mirror disk resource
        - Service resource     : Service resource for MongoDB Server (MongoDB)_service
...
### **4. Start group on Primary server**
       ...
       | Parameter            | Value                       |
       +----------------------+-----------------------------+
       | Name                 | MongoDBfailover             |
       | Startup Server       | ecxcluster1 -> ecxcluster2  |
       | Floating IP Address  | 192.168.1.3                 |
       | Mirror Disk resource | D:\data                     |
       +----------------------+-----------------------------+

 If want to know how to add the resource, please refer to "[EXPRESSCLUSTER X 5.3 for Windows Installation and Configuration Guide](https://docs.nec.co.jp/software/clustering/expresscluster_x/x53/ecx_x53_windows_en/W53_IG_EN/index.html#)". 
...
- Download and Install MongoDB 8.2.6 on both the servers.
...
1. Installation Directory
    ...
    - e.g. *C:\Program Files\MongoDB\Server\8.2*
1. Choose setup type
    - Complete
1. Data Directory
    - Specify the local directory (C: drive) as **Data Directory**.
    - e.g. *C:\Program Files\MongoDB\Server\8.2\data*
1. Ready to Install
    - Click **Next**
...
- Copy MongoDB data from default directory (C:\Program Files\MongoDB\Server\8.2\data) to Mirror disk (D:\data)
...
- The MongoDB configuration file (mongod.cfg) is located at: (C:\ProgramFiles\MongoDB\Server\8.2\bin) in windows.
```
# for Node1
net:
  bindIp: "127.0.0.1,192.168.1.1,192.168.1.3"
storage:
     dbPath: D:\data
```
```
...
```
    ...
    testdb1>db.myCollection.insertOne( { x: 1 } );
    <{
    acknowledged: true,
...
```  
### **10. After moving group failover on Node 2 to check newly created Database " testdb1" for testing MongoDB database.**
